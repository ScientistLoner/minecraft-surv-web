...
```bash
npm test                # юнит-тесты (без БД)
npm run test:integration # интеграционные тесты API
npm run test:all        # всё сразу
```

Интеграционные тесты крутят Express на случайном порту и проверяют API. Тесты, которым нужна БД, автоматически пропускаются, если PostgreSQL недоступен; тест на запись в БД требует переменной `TEST_DB=1` (чтобы случайно не засорять боевую базу).
...
├── tests/                  # Юнит- и интеграционные тесты (node:test)
└── src/
